...
import com.gmail.maystruks08.nfcruntracker.ui.runners.paged.RunnerDataSource
import com.gmail.maystruks08.nfcruntracker.ui.runners.paged.RunnersDataSourceFactory
...
class RunnersViewModel @Inject constructor(
    ...
    private val dataSource: RunnerDataSource,
    private val dataSourceFactory: RunnersDataSourceFactory) : BaseViewModel() {

    val runnersList = LivePagedListBuilder(dataSourceFactory, config).build()
    ...
    private fun onRunnersUpdates(onResult: ResultOfTask<Exception, RunnerChange>) {
        when (onResult) {
            is ResultOfTask.Value -> dataSource.invalidate()
            is ResultOfTask.Error -> handleError(onResult.error)
        }
    }
