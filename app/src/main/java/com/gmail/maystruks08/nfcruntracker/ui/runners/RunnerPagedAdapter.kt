...
class RunnerPagedAdapter(private val clickListener: (RunnerView) -> Unit) : PagedListAdapter<RunnerView, RunnerPagedAdapter.ViewHolder>(DIFF_CALLBACK) {
    ...
    override fun onCreateViewHolder(parent: ViewGroup, viewType: Int): RunnerPagedAdapter.ViewHolder {
...
private val DIFF_CALLBACK = object : DiffUtil.ItemCallback<RunnerView>() {
    ...
    override fun areItemsTheSame(oldRunnerView: RunnerView, newRunnerView: RunnerView): Boolean = oldRunnerView.id == newRunnerView.id

    override fun areContentsTheSame(oldItem: RunnerView, newItem: RunnerView): Boolean = oldItem == newItem
