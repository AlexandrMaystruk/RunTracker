...
interface DistanceDAO : BaseDao<DistanceTable> {
    ...
    @Query("SELECT COUNT(*) FROM runners  WHERE actualRaceId =:raceId AND actualDistanceId =:distanceId AND isOffTrackDistance LIKE :distanceId;")
    fun getRunnerCountOffTrack(
        raceId: String,
        distanceId: String
    ): Int

    @Query("SELECT COUNT(*) FROM runners WHERE actualRaceId =:raceId AND actualDistanceId =:distanceId AND isOffTrackDistance IS NULL AND (SELECT COUNT(*) FROM checkpoints WHERE checkpoints.distanceId =:distanceId) = (SELECT COUNT(*) FROM result WHERE runners.runnerNumber = result.runnerNumber )")
